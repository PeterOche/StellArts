...
class MockHardwareStoreAPI:
    ...
    def __init__(self):
        ...
        self.stores = [
            ...
            {
                "id": "store_402",
                "name": "Lowe's - Westside",
                "lat": 40.7300,
                "lon": -74.0200,
            },
            {
                "id": "store_705",
                "name": "Ace Hardware - East",
                "lat": 40.7500,
                "lon": -73.9800,
            },
        ]

    def find_stores_near_route(
        self,
        start_lat: float,
        start_lon: float,
        end_lat: float,
        end_lon: float,
        max_deviation_miles: float = 2.0,
    ) -> list[dict]:
        """
...
class InventoryService:
    ...
    async def check_route_inventory(
        self, artisan, booking, required_materials: list[str]
    ):
        """
        ...
        for store in stores_on_route:
            ...
            if has_stock:
                ...
                logger.info(
                    f"Inventory check positive: Dispatching push notification to artisan {artisan.id}"
                )
